...
public class CourseService {
    ...
    private void validateUserActiveSesion() {
        if (!UserActiveSesion.username.equals(userAdmin)) {
            throw new IntegrityValidation("The administrator is the only one who has permissions and access to delete the course.");
        }
    }

    private void validateCourseExistsId(Long id) {
        if (!courseRepository.findById(id).isPresent()) {
            throw new IntegrityValidation("Sorry, the course was not found in the database.");
        }
    }

    private void validateCourseExistsName(String name) {
        if (courseRepository.findByName(name)) {
            throw new IntegrityValidation("This course has already been registered previously, please try again");
        }
    }
}
